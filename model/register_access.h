...
#include <bm/bm_sim/packet.h> // for Packet
...
public:
  ...
  static uint16_t
  get_clone_mirror_session_id (bm::Packet *pkt)
  {
  ...
  static void
  set_clone_mirror_session_id (bm::Packet *pkt, uint16_t mirror_session_id)
  {
    uint64_t rv = pkt->get_register (CLONE_MIRROR_SESSION_ID_REG_IDX);
  ...
  static uint16_t
  get_clone_field_list (bm::Packet *pkt)
  {
  ...
  static void
  set_clone_field_list (bm::Packet *pkt, uint16_t field_list_id)
  {
    uint64_t rv = pkt->get_register (CLONE_FIELD_LIST_REG_IDX);
  ...
  static uint16_t
  get_recirculate_flag (bm::Packet *pkt)
  {
  ...
  static void
  set_recirculate_flag (bm::Packet *pkt, uint16_t field_list_id)
  {
    uint64_t rv = pkt->get_register (RECIRCULATE_FLAG_REG_IDX);
