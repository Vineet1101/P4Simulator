...
int P4CoreV1model::AddFlowEntry(const std::string &tableName,
  ...
  if (rc != bm::MatchErrorCode::SUCCESS) {
    std::cout << "AddFlowEntry failed for table " << tableName
              << " with error code " << MatchErrorCodeToStr(rc);
    return -1;
  }
...
int P4CoreV1model::DeleteFlowEntry(const std::string &tableName,
  ...
  if (rc != bm::MatchErrorCode::SUCCESS) {
    std::cout << "DeleteFlowEntry failed for table " << tableName
              << " and handle " << handle << " with code "
              << static_cast<int>(rc) << " (" << MatchErrorCodeToStr(rc) << ")";
    return -1;
  }
...
int P4CoreV1model::SetEntryTtl(const std::string &tableName,
  ...
  if (rc != bm::MatchErrorCode::SUCCESS) {
    std::cout << "SetEntryTtl failed for table " << tableName << ", handle "
              << handle << " with code " << static_cast<int>(rc) << " ("
              << MatchErrorCodeToStr(rc) << ")";
    return -1;
  }
...
int P4CoreV1model::MeterSetRates(
...
int P4CoreV1model::MeterGetRates(
    const std::string &meterName, size_t idx,
    std::vector<bm::Meter::rate_config_t> *configs) {
  bm::Meter::MeterErrorCode rc =
...
int P4CoreV1model::MeterResetRates(const std::string &meterName, size_t idx) {
