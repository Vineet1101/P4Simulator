...
P4TopologyReader::Read()
{
    ...
    {
        NS_LOG_WARN("P2P topology file cannot be opened. Check the filename and permissions.");
        abort();
    }
    ...
    // First line specifies total nodes and links
    getline(fileStream, line);
    lineStream.str(line);

    // The frist line of the file should be the number of nodes and links
    lineStream >> switchNum >> hostNum >> linkNum;

    NS_LOG_INFO("P2P topology with " << switchNum << " switches and " << hostNum
                                     << " nodes with total " << linkNum << " links.");
    ...
    for (int i = 0; i < linkNum && !fileStream.eof(); ++i)
    {
        ...
        lineStream >> fromIndex >> fromType >> toIndex >> toType >> dataRate >> delay;

        NS_LOG_INFO("Link " << i << ": from " << fromType << fromIndex << " to " << toType
                            << toIndex << " with DataRate " << dataRate << " and Delay " << delay);

        if (nodes[fromIndex] == nullptr)
        {
            nodes[fromIndex] = CreateObject<Node>();
            NS_LOG_INFO("Created Node " << fromIndex);
            createdNodeNum++;
        }

        if (nodes[toIndex] == nullptr)
        {
            nodes[toIndex] = CreateObject<Node>();
            NS_LOG_INFO("Created Node " << toIndex);
            createdNodeNum++;
        }

        Link link(nodes[fromIndex], fromIndex, 'N', nodes[toIndex], toIndex, 'N');
    ...
    for (int i = 0; i < switchNum && !fileStream.eof(); i++)
    {
        ...
        lineStream >> switchIndex >> networkFunction;
    ...
    fileStream.close();
    NS_LOG_INFO("P2P topology successfully read.");
