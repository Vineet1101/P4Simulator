...
class P4CoreV1model : public P4SwitchCore
{
  public:
    ...
    /**
     * @brief Adds an entry to a match table with a match key, action, and optional priority
     * @param tableName Name of the match table
    ...
    int SetDefaultAction(const std::string& tableName,
                         ...
                         bm::ActionData&& actionData);
    /**
     * @brief Resets the default entry for a match table to its original configuration
     * @param tableName Name of the match table
     */
    ...
    /**
     * @brief Modifies an existing match table entry with a new action and parameters
     * @param tableName Name of the match table
    ...
    int ModifyFlowEntry(const std::string& tableName,
                        ...
                        bm::ActionData actionData);
    /**
     * @brief Sets a time-to-live (TTL) in milliseconds for a specific match table entry
     * @param tableName Name of the match table
...
/**
 * @brief Modifies an existing indirect match table entry to update its member.
 * @param tableName Name of the indirect match table.
...
/**
 * @brief Sets the default member to be used when no match is found in an indirect match table.
 * @param tableName Name of the table.
...
/**
 * @brief Adds an entry to an indirect wide-switch (WS) match table using a group handle.
...
/**
 * @brief Modifies an entry in an indirect WS match table with a new group handle.
 * @param tableName Name of the indirect WS match table.
...
int SetIndirectWsDefaultGroup(const std::string& tableName,
     ...
     * @param tableName Name of the match table
     * @param entriesOut Pointer to a vector where the retrieved entries will be stored
     * @return 0 on success, -1 on failure
     */
 ...
 * @param[out] out Pointer to output stream to write the serialized configuration.
 * @return 0 on success, -1 on failure.
 */
