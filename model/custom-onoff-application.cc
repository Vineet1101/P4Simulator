  ...
  static TypeId tid =
      TypeId ("ns3::CustomOnOffApplication")
          ...
          .AddAttribute (
              ...
              AddressValue (), MakeAddressAccessor (&CustomOnOffApplication::m_local),
              MakeAddressChecker ())
          .AddAttribute ("OnTime",
...
// Application Methods
void
CustomOnOffApplication::StartApplication () // Called at time specified by Start
{
  NS_LOG_FUNCTION (this);
...
void
CustomOnOffApplication::StopApplication () // Called at time specified by Stop
{
  NS_LOG_FUNCTION (this);
  ...
  if (m_socket != 0)
    ...
    {
      NS_LOG_WARN ("CustomOnOffApplication found null socket to close in StopApplication");
    }
}

void
CustomOnOffApplication::CancelEvents ()
{
  NS_LOG_FUNCTION (this);
...
// Event handlers
void
CustomOnOffApplication::StartSending ()
{
  NS_LOG_FUNCTION (this);
...
void
CustomOnOffApplication::StopSending ()
{
  NS_LOG_FUNCTION (this);
  CancelEvents ();
...
// Private helpers
void
CustomOnOffApplication::ScheduleNextTx ()
{
  NS_LOG_FUNCTION (this);
  ...
  if (m_maxBytes == 0 || m_totBytes < m_maxBytes)
    {
      ...
      m_sendEvent = Simulator::Schedule (nextTime, &CustomOnOffApplication::SendPacket, this);
    }
  else
...
void
CustomOnOffApplication::ScheduleStartEvent ()
...
void
CustomOnOffApplication::ScheduleStopEvent ()
...
void
CustomOnOffApplication::SetCustomHeader (const CustomHeader &header)
{
...
CustomOnOffApplication::SendPacket ()
{
  ...
  if (m_unsentPacket)
    ...
    {
      // [deprecated] This should be avoided in the future, because the p4 will check the header
      Address from, to;
      m_socket->GetSockName (from);
      m_socket->GetPeerName (to);

      SeqTsSizeHeader seqHeader;
      seqHeader.SetSeq (m_seq++);
      seqHeader.SetSize (m_pktSize);

      NS_ABORT_IF (m_pktSize < seqHeader.GetSerializedSize ());

      // Create the base packet
      packet = Create<Packet> (m_pktSize - seqHeader.GetSerializedSize ());

      // Trace before adding SeqTsSizeHeader, for consistency with PacketSink
      m_txTraceWithSeqTsSize (packet, from, to, seqHeader);

      // Add the SeqTsSizeHeader
      packet->AddHeader (seqHeader);

      // Add the custom header
      packet->AddHeader (m_customHeader);
    }
  else
    {
      // Create the base packet
...
void
CustomOnOffApplication::ConnectionSucceeded (Ptr<Socket> socket)
{
...
void
CustomOnOffApplication::ConnectionFailed (Ptr<Socket> socket)
{
  NS_LOG_FUNCTION (this << socket);
