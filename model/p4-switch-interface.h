...
public:
  ...
  // Setters
  void
  SetP4NetDeviceCore (P4Switch *model)
  {
    p4Core_ = model;
  }

  void
  SetJsonPath (const std::string &path)
  {
    jsonPath_ = path;
  }

  void
  SetP4InfoPath (const std::string &path)
  {
    p4InfoPath_ = path;
  }

  void
  SetFlowTablePath (const std::string &path)
  {
    flowTablePath_ = path;
  }

  void
  SetViewFlowTablePath (const std::string &path)
  {
    viewFlowTablePath_ = path;
  }

  void
  SetNetworkFunc (unsigned int func)
  {
    networkFunc_ = func;
  }

  void
  SetPopulateFlowTableWay (uint way)
  {
  ...
  const std::string &
  GetFlowTablePath () const
  {
    return flowTablePath_;
  }

  const std::string &
  GetViewFlowTablePath () const
  {
    return viewFlowTablePath_;
  }

  unsigned int
  GetNetworkFunc () const
  {
    return networkFunc_;
  }

  uint
  GetPopulateFlowTableWay () const
  {
