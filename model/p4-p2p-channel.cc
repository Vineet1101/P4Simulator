  ...
  static TypeId tid =
      TypeId ("ns3::P4P2PChannel")
          ...
          .AddAttribute ("Delay", "Propagation delay through the channel", TimeValue (Seconds (0)),
                         MakeTimeAccessor (&P4P2PChannel::m_delay), MakeTimeChecker ())
...
P4P2PChannel::P4P2PChannel () : Channel (), m_delay (Seconds (0.)), m_nDevices (0)
{
...
P4P2PChannel::TransmitStart (Ptr<const Packet> p, Ptr<CustomP2PNetDevice> src, Time txTime)
{
  ...
  Simulator::ScheduleWithContext (m_link[wire].m_dst->GetNode ()->GetId (), txTime + m_delay,
                                  &CustomP2PNetDevice::Receive, m_link[wire].m_dst, p->Copy ());
