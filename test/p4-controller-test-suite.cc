...
//
void
P4ControllerCheckFlowEntryTestCase::DoRun()
{
    ...
    NS_TEST_ASSERT_MSG_NE(topoReader->LinksSize(),
                          0,
    ...
    p4Helper.SetDeviceAttribute(
        "JsonPath",
        StringValue(
            "/home/p4/gsoc/ns3/contrib/p4sim/test/p4src/simple_v1model/simple_v1model.json"));
    p4Helper.SetDeviceAttribute(
        "FlowTablePath",
        StringValue("/home/p4/gsoc/ns3/contrib/p4sim/test/p4src/simple_v1model/flowtable_0.txt"));
    ...
    for (uint32_t i = 0; i < switchNode.GetN(); ++i)
    {
        ...
        {
            Ptr<P4SwitchNetDevice> p4dev = DynamicCast<P4SwitchNetDevice>(p4Devices.Get(k));
            if (p4dev)
            {
    ...
    Simulator::Schedule(Seconds(1.0), [this, &controller]() {
        NS_TEST_ASSERT_MSG_EQ(controller.GetN(), 1, "There should be only one p4switch in there");

        NS_TEST_ASSERT_MSG_EQ(
            controller.GetTableEntryCount(0, "MyIngress.ipv4_nhop"),
            2,
            "Verify that we could get the right count of table entries in the controller");

        NS_TEST_ASSERT_MSG_EQ(
            controller.GetTableEntryCount(0, "MyIngress.arp_simple"),
            2,
            "Verify that we could get the right count of table entries in the controller");
    });
...
{
  public:
    P4ControllerTestSuite()
        : TestSuite("p4-controller", UNIT)
    {
